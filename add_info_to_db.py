...
with open(os.path.join(store, "id.dat"), "r") as id_file:
	for line in id_file:
		item_id = line.strip();
		if item_id == "":
			print("ERROR: Item Id is an empty string! Item from " + store + " (line = " + count + ") is not added to the data-base.")
			continue
		count+=1
		...
		image_path = "dataset_hm/" + str(item_id) + "_0.jpg"
		...
		with open(os.path.join(html_path, filename), "r") as f:
			...
			db.insert_into_t_shirts(item_id, url, image_path, title, store_name, gender, price)
